...
################################################################################
x.train <- D[!duplicated(D$ID_t), 2:84]
colnames(x.train) <- colnames(D[,2:84]) 

ii <- unique(D[which(D$indNV==1),1]) # N=1443 individuals moved to the individual field during Wave 1 to 5
length(ii)
head(D[D$ID_t %in% ii, c(1:3, 82:87)])
...
pred <- predict.survbart(post, pre$tx.test[,-1])
...
axis(1, at=0:4, labels=c("Welle 1", "Welle 2", "Welle 3", "Welle 4", "Welle 5"))
axis(2)
lines(c(0, pre$times), c(1, sv.025), type='s', lty=2)
lines(c(0, pre$times), c(1, sv.975), type='s', lty=2)

# B. Derive posterior draws of prob. of temp. dropout, for each wave separately
